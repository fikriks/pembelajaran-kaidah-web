...
<?= $this->section('title') ?>Edit Guru - <?= $this->endSection() ?>
...
<div class="container-fluid">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold text-dark">Edit Guru</h4>
    ...
    <div class="card border-primary bg-light mb-4">
        <div class="card-body p-3">
            <div class="row align-items-center">
                ...
                <div class="col">
                    <h5 class="mb-1"><?= esc($guru['nama_lengkap']) ?></h5>
                    <p class="text-muted mb-0">
                        <strong>Username:</strong> <?= esc($guru['nama_pengguna']) ?> |
                        <strong>Role:</strong> <span class="badge bg-info">Guru</span> |
                    ...
                    <small class="text-muted">Dibuat: <?= date('d M Y H:i', strtotime($guru['waktu_dibuat'])) ?></small>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body p-4">
            ...
            <div class="alert alert-warning d-flex align-items-center" role="alert">
                ...
                <div>
                    ...
                    Isi password baru hanya jika ingin mengubah password guru.
                </div>
            </div>

            <!-- Form -->
            <form method="POST" action="<?= site_url('guru/' . $guru['id_pengguna']) ?>" class="needs-validation" novalidate>
                <?= csrf_field() ?>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="nama_pengguna" name="nama_pengguna"
                                   value="<?= old('nama_pengguna', $guru['nama_pengguna']) ?>"
                                   required minlength="3" maxlength="50"
                                   pattern="[a-zA-Z0-9\s]+" placeholder="contoh: ahmad_fauzi">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="card border-primary bg-light">
                            <div class="card-body p-3">
                                <h6 class="card-title text-primary mb-2">
                                    <i class="ti ti-school me-1"></i>Informasi Guru
                                </h6>
                                <div class="small">
                                    <p class="mb-1">
                                        <strong>Role:</strong> Guru (tidak dapat diubah)
                                    </p>
                                    <p class="mb-1">
                                        <strong>Dibuat:</strong> <?= date('d M Y H:i', strtotime($guru['waktu_dibuat'])) ?>
                                    </p>
                                    <?php if (!empty($guru['waktu_diubah'])): ?>
                                    <p class="mb-1">
                                        <strong>Diubah:</strong> <?= date('d M Y H:i', strtotime($guru['waktu_diubah'])) ?>
                                    </p>
                                    <?php endif; ?>
                                    <p class="mb-0">
                                        <strong>Guru ID:</strong> #<?= $guru['id_pengguna'] ?>
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Access Information -->
                        <div class="card border-info bg-light mt-3">
                            <div class="card-body p-3">
                                <h6 class="card-title text-info mb-2">
                                    <i class="ti ti-shield-check me-1"></i>Akses Guru
                                </h6>
                                <div class="small">
                                    <ul class="mb-0">
                                        <li>✅ Kelola materi kaidah</li>
                                        <li>✅ Kelola soal pembelajaran</li>
                                        <li>✅ Lihat progress siswa</li>
                                        <li>✅ Akses laporan pembelajaran</li>
                                        <li>❌ Kelola pengguna (admin only)</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                ...
                <div class="row mt-4">
                    <div class="col-12">
                        ...
                        <div class="d-flex gap-2 justify-content-end">
                            ...
                            <a href="<?= site_url('guru') ?>" class="btn btn-danger">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="d-grid gap-2">
                        <button type="button" class="btn btn-outline-warning" onclick="toggleGuruStatus()">
                            <i class="ti ti-toggle-left me-2"></i>
                            <?= $guru['status'] === 'AKTIF' ? 'Nonaktifkan' : 'Aktifkan' ?> Guru
                        </button>
                    </div>
                </div>
                <div class="col-md-6">
                    ...
                    <div class="d-grid gap-2">
                        <button type="button" class="btn btn-outline-danger" onclick="confirmDelete()">
                            <i class="ti ti-trash me-2"></i>Hapus Guru
                        </button>
                    </div>
                    <?php else: ?>
...
function toggleGuruStatus() {
    ...
    if (confirm(`Apakah Anda yakin ingin ${actionText} guru ini?`)) {
        fetch(`<?= site_url('guru/toggleStatus/') ?><?= $guru['id_pengguna'] ?>`, {
            method: 'POST',
            headers: {
...
function confirmDelete() {
    if (confirm('Apakah Anda yakin ingin menghapus guru ini? Tindakan ini tidak dapat dibatalkan!')) {
        ...
        form.method = 'POST';
        form.action = '<?= site_url('guru/delete/') ?><?= $guru['id_pengguna'] ?>';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const originalUsername = '<?= $guru['nama_pengguna'] ?>';
    ...
    function checkUsernameAvailability(username) {
        ...
        if (username.length >= 3) {
            usernameCheckTimeout = setTimeout(() => {
                fetch('<?= site_url('guru/checkUsername') ?>', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest'
