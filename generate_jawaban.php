...
echo "Connected to database successfully\n";
    ...
    $arabicTerms = [
        ...
        'رَفَعٌ (raf\'un)', 'نَصَبٌ (nashabun)', 'خَفَضٌ (khafdhun)', 'جَزْمٌ (jazmun)',
    ...
    for ($soalId = 31; $soalId <= 210; $soalId++) {
        // Random tingkat kesulitan (distribusi 40% mudah, 40% sedang, 20% sulit)
        $rand = rand(1, 100);
        if ($rand <= 40) {
            $tingkat = 'mudah';
        } elseif ($rand <= 80) {
        ...
        for ($i = 0; $i < 4; $i++) {
            ...
            $sql = "INSERT INTO pilihan_jawaban (id_soal, teks_jawaban, is_benar, urutan, waktu_dibuat) VALUES (?, ?, ?, ?, NOW())";
            $db->query($sql, [$soalId, $teksJawaban, $isBenar, $urutan]);
